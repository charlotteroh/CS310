...
void create_cashier(FILE* file) {
  ...
  while (fscanf(file, "%d", &sandwich) != EOF) {
    // lock board
...
void start(char* file[]) {
  ...
  for (int i=2; i <= cashier_size+1; i ++) {
    thread_create((thread_startfunc_t) create_cashier, fopen(file[i], "r"));
  }
...
int main(int argc, char* argv[]) {
  ...
  // read file
  sscanf(argv[1], "%d", &largest_num_of_orders);
